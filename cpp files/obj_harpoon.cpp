...
xPosition += speed;
yPosition += speed;
// ------- COLLISION CHECK WITH OBJECTS | source for collision code: FriendlyCosmoNaut 
// horizontal
...
if(xPosition != 0){
	if(place_meeting(x + xPosition, y, obj_oil_barrel)){ // checks if craboo's x & y position collides w/ obj_rock's collision mask
		repeat(abs(xPosition)){					   // repeating for the absolute value amount of xPosition 
			...
			else{ break; }						   //  loop ends once diver's intended xPosition + or - 1, collides w/ obj_rock
		}
...
if(yPosition !=0){
	if(place_meeting(x, y + yPosition, obj_oil_barrel)){
		repeat(abs(yPosition)){
			...
			else { break; } //  loop ends once diver's intended yPosition + or - 1, collides w/ obj_rock
		}
...
audio_play_sound(s_fish_hit, 1, false);
instance_destroy();
